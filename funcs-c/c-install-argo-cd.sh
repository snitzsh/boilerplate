...
clusterInstallArgoCD () {
  ...
  # TODO: revise `api-main` `ui-main`, both belong to snitzsh app, but what if
  #       there is another app `xxx` they will have `api-main` and currently
  #       there is not way to differenciate which belongs to which.
  ...
  dependencies=$( \
    ...
    yq \
      ...
      '
        ...
        | del($_dependencies[].charts[] | select(.name as $name | ($_ignore_repositories[] as $filter | $name | contains($filter))))
        | $_dependencies
  ...
  #  make sure to use snitzsh-api-main, instead of api-main
  #
      ...
      '
        [
          (
            ...
            | .[]
            | ("helm-chart-" + . + "-configs") as $repository_name
            | {
  ...
  yq \
    ...
    '
      ...
      | . as $init
      | .[$_chart_name].configs.repositories = {}
